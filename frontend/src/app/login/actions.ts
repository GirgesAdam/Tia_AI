...
export async function loginAction(formData: FormData) {
  ...
  if (!response.ok) { await supabase.auth.signOut(); redirect(`/login?error=${encodeURIComponent("الحساب مش مربوط بـ Tia AI بشكل صحيح.")}`); }
  const me = await response.json() as {workspaces:Array<{workspace_id:string}>};
  if (!me.workspaces.length) { await supabase.auth.signOut(); redirect(`/login?error=${encodeURIComponent("الحساب مش عضو في أي Workspace.")}`); }
