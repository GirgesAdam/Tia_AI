...
def _bookable_context(db: Session, workspace: Workspace, *, service_slug: str | None = None, exclude_service_id: str | None = None):
    ...
    if service_slug and exact_required:
        required_id = db.execute(
            text("SELECT id FROM services WHERE workspace_id=:wid AND slug=:slug AND is_active IS TRUE"),
            {"wid": workspace.id, "slug": service_slug},
        ).scalar_one_or_none()
