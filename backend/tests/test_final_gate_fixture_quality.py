...
def test_final_gate_fixture_source_uses_approved_synthetic_data() -> None:
    ...
    source = (
        backend / "scripts" / "seed_final_internal_gate.py"
...
def test_full_staging_contacts_are_safe_reserved_test_contacts() -> None:
    backend = Path(__file__).resolve().parent.parent
    source = (
        backend / "scripts" / "seed_full_staging_demo.py"
    ).read_text(encoding="utf-8")
    ...
    assert "+200000" in source
    assert ".tia.example" in source


def test_data_quality_validator_checks_runtime_rows() -> None:
    backend = Path(__file__).resolve().parent.parent
    source = (
        backend / "scripts" / "validate_final_gate_fixture_quality.py"
    ).read_text(encoding="utf-8")
    ...
    assert 'patient.email.endswith(".example")' in source
