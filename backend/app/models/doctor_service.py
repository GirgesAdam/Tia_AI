...
from sqlalchemy import Boolean, CheckConstraint, ForeignKeyConstraint, Integer, UniqueConstraint, text
...
class DoctorService(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "doctor_id", "service_id", name="uq_doctor_services_assignment"),
        ...
        CheckConstraint("custom_duration_minutes IS NULL OR custom_duration_minutes > 0", name="doctor_service_duration_valid"),
        CheckConstraint("custom_price_minor IS NULL OR custom_price_minor >= 0", name="doctor_service_price_non_negative"),
    )
