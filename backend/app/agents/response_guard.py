...
_DOCTOR_SHORT_ID = re.compile(
    r"(?i)\b(الدكتور|الطبيب|doctor)\s+[0-9a-f]{8,32}\b"
)

_BRANCH_SHORT_ID = re.compile(
    r"(?i)\b(الفرع|branch)\s+[0-9a-f]{8,32}\b"
)

_SERVICE_SHORT_ID = re.compile(
    r"(?i)\b(الخدمة|service)\s+[0-9a-f]{8,32}\b"
)
