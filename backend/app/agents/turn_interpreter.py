...
def _interpreter_system_prompt(
    *,
    ...
    return (
        ...
        "VERIFIED OPTION REFERENCES: in an active flow, a verified service/doctor option set in workflow "
        "options remains a valid reference target across one harmless side read or factual answer. Resolve a "
        "short positional or pronominal follow-up from the recent conversation, but ground the choice only to "
        "that persisted verified option set. An intervening factual answer is not a new option list and must "
        "not steal the reference. If more than one verified option still fits, preserve ambiguity instead of "
        "guessing.\n\n"
        ...
        "clock time from a presented availability window in a context that authorizes booking, include "
        ...
        "booking flow currently lists availability_discovery only; the latest contextual or explicit booking "
        "authorization owns the capability for that turn.\n\n"
        ...
        "APPOINTMENT CANCELLATION: include appointment_cancellation when the latest turn requests cancellation, "
        ...
        "refresh availability for them. FRESH RESCHEDULE AUTHORIZATION: on the first turn of a "
        ...
        "For a presented replacement availability window, use the same semantic distinction as booking: if "
        "the assistant asked the customer to choose one replacement start and the latest turn chooses it, use "
        "action=select_option with selection_time=HH:MM and appointment_reschedule. If the assistant instead "
        "asks when the replacement search should begin/end, or the customer requests a broader time constraint, "
        "use action=modify with not_before_time/not_after_time. Decide from conversational context, not surface "
        "wording alone. If the same exact time belongs to multiple presented doctors and the latest turn does "
        "not single out one doctor, preserve the doctor ambiguity and do not mark an exact slot selected. "
