...
This script exercises Tia's configured production LLM through the real semantic
interpreter and the real clinic catalog. It performs no appointment/package writes.
"""
...
def main() -> int:
    ...
    with Session(engine) as db:
        ...
        failures = 0
        print("Running exactly 5 live compound semantic conversations")
        for index, case in enumerate(_cases(), start=1):
            ...
            print(f"[{status}] {index}/5 {case.name}")
            print(f"  customer: {latest}")
            print(f"  capabilities: {list(decision.capabilities or [])}")
            print(f"  {details}")
        print(f"Summary: {5 - failures} passed, {failures} failed")
