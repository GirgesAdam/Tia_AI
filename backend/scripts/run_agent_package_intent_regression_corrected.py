...
def case_existing_package_wants_new_package_for_other_service(**ctx) -> CaseResult:
    ...
    try:
        ...
        ok = (
            ...
            and not assessment.says_existing_active_package_blocks_new
        ...
        return _case(
            ...
            error=(
                "A package for a different service must not block a new package request. "
                "The existing package must remain untouched, the request must stay package "
                "intent, and it must not be turned into a single appointment."
            ),
        )
    except Exception as exc:
        return _case(
